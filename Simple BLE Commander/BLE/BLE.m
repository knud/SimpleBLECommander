...
-(void) read
{
  CBUUID *uuid_service = [CBUUID UUIDWithString:@SIMPLE_COMMAND_SERVICE_UUID];
  CBUUID *uuid_char = [CBUUID UUIDWithString:@SIMPLE_COMMAND_INVOKE_CHARACTERISTIC_UUID];
...
{
  CBUUID *uuid_service = [CBUUID UUIDWithString:@SIMPLE_COMMAND_SERVICE_UUID];
  CBUUID *uuid_char = [CBUUID UUIDWithString:@SIMPLE_COMMAND_RESPONSE_CHARACTERISTIC_UUID];
//  CBUUID *uuid_char = [CBUUID UUIDWithString:@SIMPLE_COMMAND_INVOKE_CHARACTERISTIC_UUID];
...
//    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@SIMPLE_COMMAND_SPARE_CHARACTERISTIC_UUID]])
//      NSLog(@"didUpdateValueForCharacteristic for %@",@SIMPLE_COMMAND_SPARE_CHARACTERISTIC_UUID);
//
//    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@SIMPLE_COMMAND_INVOKE_CHARACTERISTIC_UUID]])
//      NSLog(@"didUpdateValueForCharacteristic for %@",@SIMPLE_COMMAND_INVOKE_CHARACTERISTIC_UUID);

    // We use this characteristic to exchange messages with the peripheral, so pass the update
    // along to the delegate
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@SIMPLE_COMMAND_RESPONSE_CHARACTERISTIC_UUID]]) {
      NSLog(@"didUpdateValueForCharacteristic for %@",@SIMPLE_COMMAND_RESPONSE_CHARACTERISTIC_UUID);
      if (characteristic.value) {
        NSLog(@"read value got %lu data",(unsigned long)[characteristic.value length]);
